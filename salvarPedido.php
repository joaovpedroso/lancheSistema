...
if ($_POST) {
    ...
    foreach ($_SESSION["produtos"] as $prods => $quantidade) {
        ...
        while ($dados = $resultado->fetch(PDO::FETCH_OBJ)) {
            $preco = $dados->preco;
            $qtd = $quantidade;
            $vendidos = $dados->quantidadeVendida;
            ...
            $sqlP = "UPDATE produto SET quantidadeVendida = ? WHERE id = ? LIMIT 1";
            $resultado = $pdo->prepare($sqlP);
            $resultado->bindParam(1, $vendidos);
            $resultado->bindParam(2, $prods);
            ...
            if ($resultado->execute()) {
                ...
                echo "<script>alert('ERRO: O pedido não pode ser finalizado. Entre em contato com nossa empresa'); location.href='index.php';</script>";
            }
        }
    }
}

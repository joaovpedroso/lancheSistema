...
if ($_POST) {
    ...
    if (empty($ativo)) {
        echo "<script>alert('Selecione o Status do Usuário'); history.back();</script>";
        exit;
    }
    ...
    if (empty($id)) {
        ...
        while ($dados = $consulta->fetch(PDO::FETCH_OBJ)) {
            ...
            if ($cpf == $verificaCpf) {
                ...
                exit;
            } else if ($email == $verificaEmail) {
                echo "<script>alert('Email Já Cadastrado'); history.back();</script>";
                exit;
            }
        }

        $sql = "INSERT INTO usuario ( id, nome, cpf, rg, email, senha, telefone, cep, endereco, numero, cidade, id_tipo, ativo ) VALUES ( NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 3, ? ) ";
    ...
    if ($consulta->execute()) {
        echo "<script>alert('Usuario Salvo Com Sucesso ! '); location.href='listarUsuario.php';</script>";
    } else {
